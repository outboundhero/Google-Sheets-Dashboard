...
- **Webhooks (no auth — middleware-exempt)**: `POST /api/webhooks/bison-reconnect/[instance]` — receives Bison's `EMAIL_ACCOUNT_RECONNECTED` event; re-applies the sender's last-known tags (from `deliverability_inboxes.tags`) back into that Bison instance, since Bison wipes tags on reconnect. One URL per instance.
...
- **Webhooks (unauthenticated by middleware; URL is the secret)**: `POST /api/webhooks/bison-reconnect/[instance]` — receives Bison's `EMAIL_ACCOUNT_RECONNECTED` event, looks up the sender's last-known tags from `deliverability_inboxes.tags`, and re-applies them via `POST /tags/{tagId}/sender-emails`. One URL per Bison instance.
