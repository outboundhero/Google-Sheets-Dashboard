...
- **The Clients page does NOT respect the Instance Switcher (group/tier).** [clients/page.tsx](src/app/(dashboard)/clients/page.tsx) builds its list from `useSheets()` + `useAllLeads()` only — neither is instance-scoped, and the page never calls `useInstance()`. So the same client count shows for every group. A client-tag → group map **does** exist ([client-tag-allocations.ts](src/lib/client-tag-allocations.ts): sheet col A = Group 1, col C = Group 2 → Redis), but it's currently only consumed by the cron, its API route, and **/settings** — not by the Clients list filtering.
