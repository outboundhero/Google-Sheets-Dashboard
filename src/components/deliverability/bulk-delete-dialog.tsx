...
import { Loader2, AlertTriangle, Trash2, Copy, Clock } from "lucide-react";
...
export function BulkDeleteDialog({
  ...
  useEffect(() => {
    if (open) { setPicked(new Set(defaultInstances)); setResult(null); setError(null); setConfirmMode(null); setScheduled(null); }
  ...
  // Schedule a delayed delete instead of deleting now (fired by the
  // fire-scheduled-deletions cron once the grace elapses).
  const scheduleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/deliverability/bulk-delete?${instancesQuery}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "schedule", domains: selectedDomains.map((d) => d.domain), days: SCHEDULE_DAYS }),
      });
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(data.error || `HTTP ${res.status}`);
      setScheduled({ count: selectedDomains.length, days: SCHEDULE_DAYS });
      onSuccess();
    } catch (e) {
      ...
      <DialogContent className="max-w-lg max-h-[85vh] flex flex-col">
        ...
        {result ? (
          ...
          <div className="py-4 space-y-3 text-center">
            <div className="flex items-center justify-center gap-2 font-medium text-emerald-500">
              <Clock className="h-4 w-4" /> Scheduled for deletion
            </div>
            <div className="text-sm text-muted-foreground">
              {scheduled.count} domain{scheduled.count !== 1 ? "s" : ""} will be deleted from{" "}
              <span className="font-medium text-foreground">
                {[...picked].map((s) => INSTANCE_SHORT_LABELS[s as BisonInstanceSlug] ?? s).join(", ")}
              </span>{" "}
              in {scheduled.days} days. This runs automatically — nothing else to do.
            </div>
            <div className="flex justify-center">
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-3">
              ...
              <div className="text-sm">
                ...
                <span className="font-semibold">{selectedDomains.length} domain{selectedDomains.length !== 1 ? "s" : ""}</span>{" "}
                {confirmMode === "now" ? "now" : `after ${SCHEDULE_DAYS} days`} from{" "}
                <span className="font-semibold">
                  {[...picked].map((s) => INSTANCE_SHORT_LABELS[s as BisonInstanceSlug] ?? s).join(", ") || "—"}
                </span>?{" "}
                {confirmMode === "now"
                  ? "This permanently deletes their inboxes from EmailBison and removes them from LeadSync, and cannot be undone."
                  : `They'll be deleted automatically in ${SCHEDULE_DAYS} days.`}
              </div>
            </div>
            {progress && <div className="text-xs text-muted-foreground">{progress}</div>}
            {error && <div className="text-xs text-destructive">{error}</div>}
            <div className="flex items-center justify-end gap-2 pt-1">
              <Button variant="ghost" size="sm" onClick={() => { setConfirmMode(null); setError(null); }} disabled={deleting}>
                Back
              </Button>
              <Button
                variant="destructive"
                size="sm"
                disabled={deleting}
                onClick={() => (confirmMode === "now" ? runDelete(selectedDomains) : scheduleDelete())}
              >
                {deleting && <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" />}
                {confirmMode === "now" ? `Yes, delete ${selectedDomains.length}` : `Yes, schedule in ${SCHEDULE_DAYS} days`}
              </Button>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-3">
              ...
              <div className="text-sm">
                ...
                from EmailBison and removes them from LeadSync — now or in {SCHEDULE_DAYS} days. Copies on other instances stay put.
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              ...
              <Button
                ...
                title={picked.size === 0 ? "Pick at least one instance to delete from" : `Schedule deletion in ${SCHEDULE_DAYS} days`}
              >
                <Clock className="h-3.5 w-3.5 mr-1.5" /> Delete in {SCHEDULE_DAYS} days
              </Button>
              <Button
                variant="destructive"
                size="sm"
              ...
              >
                <Trash2 className="h-3.5 w-3.5 mr-1.5" /> Delete now
              </Button>
            </div>
          </div>
        )}
