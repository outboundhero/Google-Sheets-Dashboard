...
interface InstanceContextValue {
  group: BisonGroup;
  tier: TierFilter;
  setGroup: (g: BisonGroup) => void;
  setTier: (t: TierFilter) => void;
  /** Instances matched by current (group, tier). 1 entry if tier is b2b/b2c, 2 if "all". */
...
function loadFromStorage(): { group: BisonGroup; tier: TierFilter } {
  if (typeof window === "undefined") return { group: DEFAULT_GROUP, tier: "all" };
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return { group: DEFAULT_GROUP, tier: "all" };
    const parsed = JSON.parse(raw) as { group?: number; tier?: string };
    const group: BisonGroup = parsed.group === 2 ? 2 : 1;
...
export function InstanceProvider({ children }: { children: React.ReactNode }) {
  const [group, setGroupState] = useState<BisonGroup>(DEFAULT_GROUP);
  ...
  const persist = useCallback((g: BisonGroup, t: TierFilter) => {
  ...
  const instances = useMemo(() => {
    const groupInstances = instancesInGroup(group);
