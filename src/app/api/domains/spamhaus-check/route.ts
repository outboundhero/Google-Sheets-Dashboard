...
import { checkSpamhausDbl, type SpamhausResult } from "@/lib/spamhaus-dbl-resolver";
import { findBlacklistTargets, writeBlacklistResults } from "@/lib/domain-blacklist-write";

// POST /api/domains/spamhaus-check  (admin-only via middleware)
// Body: { domains: string[] } → Spamhaus DBL check for any domain. Writes
// definite results to domain_inventory and/or porkbun_domains. Returns per-domain
// { status } for the bulk-op panel (inconclusive = "failed" so Retry re-runs it).
...
export async function POST(request: Request) {
  try {
    ...
    const byDomain = new Map<string, SpamhausResult>();
    for (let i = 0; i < domains.length; i += CONCURRENT) {
      ...
      batch.forEach((d, j) => {
        const r = settled[j];
        byDomain.set(d, r.status === "fulfilled" ? r.value : { domain: d, blacklisted: null, category: null, error: "unknown" });
      });
    }

    const definite = new Map<string, boolean>();
    for (const [d, r] of byDomain) if (r.blacklisted !== null) definite.set(d, r.blacklisted);
    await writeBlacklistResults(supabase, "spamhaus", definite, targets);
    ...
    const results = domains.map((d) => {
      ...
      return r && r.blacklisted !== null
        ...
        : { domain: d, status: "failed" as const, error: r?.error || "inconclusive (denied / DNS)" };
    });
