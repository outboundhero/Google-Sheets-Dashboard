...
// DNS lookups are stateless + cheap — push concurrency high. A ~1000-domain
// batch finishes in single-digit seconds.
...
export async function POST(request: Request) {
  try {
    ...
    // (instance, domain) row so the same domain in multiple instances gets
    // each of its rows updated.
    ...
    // Resolve each unique domain once (DNS doesn't care about instance).
    const resultByDomain = new Map<string, BlacklistResult>();
    for (let i = 0; i < domains.length; i += CONCURRENT) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        resultByDomain.set(
          ...
          r.status === "fulfilled"
            ? r.value
            : { domain: batch[j], blacklisted: null, lists: [], error: "unknown" },
        );
      }
    }

    // Only write rows we got a definite answer for — inconclusive (rate-limit /
    // DNS error) results leave the existing value alone so the next run retries.
    ...
    const upsertRows = targetRows
      ...
      .filter((r): r is { instance: string; domain: string; blacklisted: boolean; blacklist_checked_at: string } => r !== null);

    for (let i = 0; i < upsertRows.length; i += 200) {
      const { error } = await supabase
        .from("deliverability_domains")
        .upsert(upsertRows.slice(i, i + 200), { onConflict: "instance,domain", ignoreDuplicates: false });
      if (error) console.error("[check-blacklist] upsert failed:", error.message);
    }

    const results = domains.map(
      (d) => resultByDomain.get(d) || { domain: d, blacklisted: null, lists: [], error: "not checked" },
    );
